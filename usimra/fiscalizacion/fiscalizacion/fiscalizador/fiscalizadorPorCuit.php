...
while($ano<=$anofin) {
	...
	foreach ($arrayPerdiosAFiscali as $perido){
		...
		if ($doit == 1) {
			...
			$idBuscador = $ano.$mes;
			if (!array_key_exists($idBuscador, $arrayAcuerdos) && !array_key_exists($idBuscador, $arrayJuicios) && !array_key_exists($idArray, $arrayRequerimientos) && !array_key_exists($idBuscador, $arrayRequerimientos) && !array_key_exists($idBuscador, $arrayPagosAnteriores)) {
