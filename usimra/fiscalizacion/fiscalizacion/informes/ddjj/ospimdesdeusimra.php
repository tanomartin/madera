...
if (isset($_POST['periodo'])) {	
	...
	$sqlOSPIM = "SELECT count(DISTINCT cuit) as cantidad, sum(totalremundeclarada)+sum(totalremundecreto) as totremun
				 FROM cabddjjospim c WHERE anoddjj = ".$arrayPeriodo[0]." and mesddjj = ".$arrayPeriodo[1]." and cuit in $whereIn";
	$resOSPIM = mysql_query($sqlOSPIM,$db);
	$rowOSPIM = mysql_fetch_assoc($resOSPIM);
	
	$sqlOSPIMDet = "SELECT count(DISTINCT cuil) as cantidad
				 FROM detddjjospim c WHERE anoddjj = ".$arrayPeriodo[0]." and mesddjj = ".$arrayPeriodo[1]." and cuit in $whereIn";
...
<body bgcolor="#B2A274">
	<div align="center">
		...
		<h3>Cantidad de DDJJ OSPIM de CUIT que presentaron USIMRA</h3>
		<form id="consultaOSUS" name="consultaOSUS" method="post" action="ospimdesdeusimra.php" onsubmit="return validar(this)" class="nover">
			<b>Periodo: </b>
