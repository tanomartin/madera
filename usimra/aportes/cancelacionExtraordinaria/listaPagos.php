...
?>
		...
		<table id="resultados" class="tablesorter" style="font-size:14px; text-align:center">
			...
			<tbody>
<?php
$sqlListaPagos="SELECT s.mespago, p.descripcion AS mesnombre, s.anopago, s.nropago, s.fechapago, s.montopagado, s.sistemacancelacion FROM cuotaextraordinariausimra s, periodosusimra p WHERE s.cuit = '$cuit' AND ((s.anopago > $anoinicio and s.anopago <= $anofin) or (s.anopago = $anoinicio and s.mespago >= $mesinicio)) AND s.anopago = p.anio AND s.mespago = p.mes ORDER BY s.anopago DESC, s.mespago ASC, s.nropago DESC";
