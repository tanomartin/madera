...
ORDER BY j.codidelega, s.cuit, s.anopago, s.mespago, s.fechapago";
	...
	if($resLeeAportes) {
		foreach($resLeeAportes as $aportes) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, invertirFecha($aportes[fechapago]));
			...
			$objPHPExcel->getActiveSheet()->setCellValue('L'.$fila, $aportes[disgdinero]);
			...
			$objPHPExcel->getActiveSheet()->setCellValue('P'.$fila, invertirFecha($aportes[fechaacreditacion]));
		}
	}
