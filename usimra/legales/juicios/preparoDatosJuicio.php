...
if ($acuAbs == 1) {
	...
	$sqlUpdateAcu = "UPDATE cabacuerdosusimra SET estadoacuerdo = 4, observaciones = '$nuevaObser', fechamodificacion = '$fechamodificacion', usuariomodificacion = '$usuariomodificacion' WHERE cuit = '$cuit' and nroacuerdo = $nroacuerdo";
} else {
	$nroacuerdo = 0;
}	
...
for ($i = 0; $i <= $peridosHabili; $i++) {
	...
	if ($_POST[$mesnombre] != "" && $_POST[$anionombre] != "") {
		...
		if ($id == '') {
			$sqlInsert = "INSERT INTO detjuiciosusimra VALUES($nroorden,'',$anio,$mes,0,'')"; 
		} else {
