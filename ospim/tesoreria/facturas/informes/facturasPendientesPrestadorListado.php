...
$arrayFacturas = array();
$sqlFacturasSinCierre = "SELECT f.*, DATE_FORMAT(fechacomprobante, '%d/%m/%Y') as fechacomprobante, DATE_FORMAT(fecharecepcion, '%d/%m/%Y') as fecharecepcion
						FROM facturas f
						WHERE idPrestador = $codigo and fechacierreliquidacion = '0000-00-00 00:00:00'";
$resFacturasSinCierre = mysql_query($sqlFacturasSinCierre,$db);
$canFacturasSinCierre = mysql_num_rows($resFacturasSinCierre);
if ($canFacturasSinCierre > 0) {
	while ($rowFacturasSinCierre = mysql_fetch_assoc($resFacturasSinCierre)) {
		$arrayFacturas[$rowFacturasSinCierre['id']] = $rowFacturasSinCierre;
	}
}

$sqlFacturas = "SELECT f.*,DATE_FORMAT(fechacomprobante, '%d/%m/%Y') as fechacomprobante, DATE_FORMAT(fecharecepcion, '%d/%m/%Y') as fecharecepcion
				FROM facturas f
				WHERE idPrestador = $codigo and 
					  fechacierreliquidacion != '0000-00-00 00:00:00' and 
					  restoapagar > 0";
...
<body bgcolor="#CCCCCC">
	<div align="center">
		<h3>Informe de Facturas Pendientes de Pago </h3>
