...
if (isset($_POST['dato']) || isset($_GET['dato'])) {
	...
	if ($filtro == 0) {
		...
		$sqlOrdenesCabecera = "SELECT o.*,p.cuit, DATE_FORMAT(o.fechaorden, '%d-%m-%Y') as fechaorden FROM prestadores p, ordencabecera o WHERE o.nroordenpago = $dato and o.codigoprestador = p.codigoprestador and p.personeria != 5";
	} 
	if ($filtro == 1) {
		$cartel = "<b>Código Prestador:<font color='blue'> $dato</font></b>";
		$sqlOrdenesCabecera = "SELECT o.*,p.cuit, DATE_FORMAT(o.fechaorden, '%d-%m-%Y') as fechaorden FROM prestadores p, ordencabecera o WHERE o.codigoprestador = $dato and o.codigoprestador = p.codigoprestador and p.personeria != 5 order by o.nroordenpago DESC";
	} 
	if ($filtro == 2) {
		$cartel = "<b>C.U.I.T.:<font color='blue'> $dato</font></b>";
		$sqlOrdenesCabecera = "SELECT o.*,p.cuit, DATE_FORMAT(o.fechaorden, '%d-%m-%Y') as fechaorden FROM prestadores p, ordencabecera o WHERE p.cuit = $dato and o.codigoprestador = p.codigoprestador and p.personeria != 5 order by o.nroordenpago DESC";
	}
