			...
			if(!$resLeeTablas){
				...
				if(stripos($tabla,"empresa") !== FALSE) {
					...
					foreach($resLeeTablas as $contenidoTabla){
						...
						$telefono = $contenidoTabla['telefono1'];
						if ($telefono == "") {
							$telefono = 0;
						}
						$obliusi = $contenidoTabla['iniobliusi'];
						if ($obliusi == "") {
							$obliusi = "0000-00-00";
						}
						$registroTabla = '('.$contenidoTabla['codidelega'].',"'.$contenidoTabla['cuit'].'","'.addslashes($contenidoTabla['nombre']).'","'.addslashes($contenidoTabla['domireal']).'","'.addslashes($contenidoTabla['nomlocali']).'",'.$contenidoTabla['codzeus'].','.$contenidoTabla['numpostal'].','.$telefono.',"'.$obliusi.$finRegistro;
...
<body bgcolor="#CCCCCC">
<div align="center">
  	<p><input type="button" name="volver" value="Volver" onclick="location.href = 'menuActualizacionUsimra.php'" /></p>
  	<h3>Resultado de la Generacion de Archivos Intranet U.S.I.M.R.A.</h3>
