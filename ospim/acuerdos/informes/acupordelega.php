...
if (isset($_POST['delega'])) {
	...
	if ($canConsulta > 0) {
		...
		$arrayResultadoSinEmpresa = array();
		$i = 0;
		$x = 0;
		while($rowConsulta = mysql_fetch_assoc($resConsulta)) {
			...
			if ($delegacion != '0') {
				if (isset($jurisdiccion[$cuit])) {
					...
					$arrayResultadoSinEmpresa[$x] = $rowConsulta;
					$x++;
				}
			} else {
				if (isset($jurisdiccion[$cuit])) {
...
<div align="center">
	...
	<?php if (isset($_POST['delega'])) { ?>
			<h3>Resultado de Busqueda de Acuerdos</h3> 
	<?php	if ($x>0) { ?>
			<h3 style="color: blue">Acuerdos de empresas inexistentes</h3>
					<table class="tablesorter" id="listadoSinEmpre" style="width:1100px; font-size:14px; text-align: center">
						<thead>
							<tr>
								<th>C.U.I.T.</th>
								<th>Nro.</th>
						...
						<tbody>
						 <?php foreach ($arrayResultadoSinEmpresa as $resultadosinempre) { ?>
						 	<tr>
				    			<td><?php echo $resultadosinempre['cuit']?></td>
				    			<td><?php echo $resultadosinempre['nroacuerdo']?></td>
				    			<td><?php echo $resultadosinempre['fechaacuerdo']?></td>
				    			<td><?php echo $resultadosinempre['tipoacuerdo']?></td>
				    			<td><?php echo $resultadosinempre['estado']?></td>
			    			</tr>
						<?php } ?>
						</tbody>
					</table>
