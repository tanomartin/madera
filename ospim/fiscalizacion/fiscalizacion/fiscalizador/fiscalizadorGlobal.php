...
function encuentroPagos($cuit, $anoinicio, $mesinicio, $anofin, $mesfin, $db) {
	$sqlPagos = "select anopago, mespago, fechapago, debitocredito, sum(importe) from afipprocesadas where cuit = $cuit and concepto != 'REM' and (concepto = '401' or concepto = '381') and ((anopago > $anoinicio and anopago <= $anofin) or (anopago = $anoinicio and mespago >= $mesinicio)) group by anopago, mespago, debitocredito, fechapago order by anopago, mespago, fechapago";
...
function deudaNominal($arrayFinal) {
	...
	foreach ($arrayFinal as $perido){
		if($perido['estado'] == "A") {
			...
			$totalDeuda = $totalDeuda + $valor81;
		}
		if ($perido['estado'] == "M") {
			$valor81 = (float)($perido['remu'] * $alicuota );
			$diferencia = $valor81 - $perido['importe'];
