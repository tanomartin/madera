...
if(strcmp($ordenbusqueda, "nroafiliado")==0) {
	$errorbusqueda = 1;
	$pagina = "moduloHiv.php?nroAfi=$nroafiliado&tipAfi=A";
}

if(strcmp($ordenbusqueda, "nrodocumento")==0) {
	$errorbusqueda = 2;
	if(strcmp($tipafiliado, 'T')==0) {
		$pagina = "moduloHiv.php?nroAfi=$nroafiliado&tipAfi=$tipafiliado";
	} else {
		$pagina = "moduloHiv.php?nroAfi=$nroafiliado&nroOrd=$nroorden&tipAfi=$tipafiliado";
	}
}
...
if ($noexistebeneficiario == 1)
	header("Location: moduloHiv.php?err=$errorbusqueda");
else
