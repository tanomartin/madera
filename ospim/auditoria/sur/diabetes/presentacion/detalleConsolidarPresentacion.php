...
$canEliminar = $canDetalle - $control;
if ($canDetalle > 0) {
	$arrayBeneDiab = array();
	$whereIn = "(";
...
function validar(formulario, cantEliminar) {
	console.log(cantEliminar);

	var beneCheck = 0;
	beneficiarios = formulario.beneficiarios;
	if (beneficiarios != null) {
		for (var x=0;x<beneficiarios.length;x++) {
			if(beneficiarios[x].checked) {
				beneCheck++;
			}
		}
	}
	console.log(beneCheck);
	if (beneCheck != cantEliminar) {
		alert("La cantidad de benficiarios no concuerda con lo especificado en la presentacion. Debe eliminar "+cantEliminar+" beneficiario/s del listado");
		return false;	
	}
...
<div align="center">
	...
	<h2>Consolidación Diabetes Detalle S.S.S.</h2>
	<?php $estado = "ACTIVA"; 
		  if ($rowCabecera['fechacancelacion'] != NULL) { $estado = "CANCELADA"; } 
		  if ($rowCabecera['fechadevolucion'] != NULL) { $estado = "FINALIZADA"; } ?>
	<h3>ID: <?php echo $rowCabecera['id'] ?> - PERIODO: <?php echo $rowCabecera['periodo'] ?> - ESTADO: <?php echo $estado." [".$control."]" ?></h3>
	<h3>CANTIDAD DETALLADO [<?php echo $canDetalle ?>]</h3>
<?php if ($canDetalle > 0) {?>	
	   <form name="consolidaDetalle" action="detalleConsolidarPresentacionGuardar.php" id="consolidaDetalle" method="post" onsubmit="return validar(this,<?php echo $canEliminar?>)" >
			<input style="display: none" type="text" value="<?php echo $rowCabecera['id']?>" name="idpres" id="idpres"'/>
			<table style="text-align:center; width:800px;" id="listado" class="tablesorter" >
				...
				<tbody>
		  <?php $i = 0;
		  		foreach ($arrayDetalle as $key => $detalle) { $arrayKey = explode("-",$key);  $i++; ?>
		  			<tr>
						...
						<td><input type="checkbox" id="beneficiarios" name="bene<?php echo $i?>" value="<?php echo $detalle['cuil']?>"/></td>
					</tr>
		  <?php } ?>
		  		</tbody>
			</table>
			<input type="submit" value="CONSOLIDAR DETALLE" name="submit"/>
		</form>
<?php } else { ?>
