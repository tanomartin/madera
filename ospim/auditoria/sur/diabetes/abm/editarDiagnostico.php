...
function validar(formulario) {
	...
	if(formulario.fechadiagnostico.value == "") {
		...
		if(!FechaValida(formulario.fechadiagnostico.value)) {
			var cajadialogo = $('<div title="Aviso"><p>La Fecha del Diagnostico ingresada no es válida.</p></div>');
			cajadialogo.dialog({modal: true, height: "auto", show: {effect: "blind",duration: 250}, hide: {effect: "blind",duration: 250}, closeOnEscape:false, close: function(event, ui) { $('#fechacomprobante').focus(); }});
			formulario.guardar.disabled = false;
			return false;
		}
							 ...
							 <p>							 </p>
							  <span class="style_texto_input"><strong>Tipo Diabetes:</strong>
								  <select name="tipodiabetes" id="tipodiabetes" class="style_input">
									...
									<?php 
									if($rowLeeDiagnostico['tipodiabetes'] == 1)
							  ...
							  <input name="fechadiagnostico" type="text" id="fechadiagnostico" value="<?php echo invertirFecha($rowLeeDiagnostico['fechadiagnostico']) ?>" size="12" placeholder="DD-MM-AAAA" class="style_input"/>
							  </span>
							  <span class="style_texto_input"><strong>Edad al Diagnostico:</strong>
								<input name="edaddiagnostico" type="text" id="edaddiagnostico" value="<?php echo $rowLeeDiagnostico['edaddiagnostico'] ?>" size="5" maxlength="3" class="style_input"/>
							  </span>
							  <p>							  </p>
							  <span class="style_texto_input"><strong>Familiares DBT en Primer Grado:</strong>
								  <select name="familiaresdbt" id="familiaresdbt" class="style_input">
									<option title="Seleccione un valor" value="">Seleccione un valor</option>
									<?php 
									if($rowLeeDiagnostico['familiaresdbt'] == 1)
