...
<body bgcolor="#CCCCCC">
	<div align="center">
		<input class="nover" type="button" name="volver" value="Volver" onclick="location.href = '../moduloDiabetes.php'" /> 
	</div>
	...
	<div align="center">
		<table id="beneficiarios" class="tablesorter" style="font-size:14px; text-align:center">
			...
			<tbody>
		<?php
		$sqlTitularActivo = "SELECT p.nroafiliado, t.apellidoynombre, t.nrodocumento, t.cuil, p.diagnosticos FROM diabetesbeneficiarios p, titulares t WHERE p.nroorden = 0 AND p.nroafiliado = t.nroafiliado ORDER BY p.nroafiliado";
		...
		while($rowTitularActivo = mysql_fetch_array($resTitularActivo)) { ?>
			<tr>
				...
				<td>Activo</td>
				<td><?php if($rowTitularActivo['diagnosticos']!=0) {?>
							<input class="nover" type="button" id="listadiagnosticos" name="listadiagnosticos" value="Editar" onclick="location.href = 'listarDiagnosticos.php?nroAfi=<?php echo $rowTitularActivo['nroafiliado']?>&nroOrd=0&estAfi=A'"/>
					<?php } else {?>
							<input class="nover" type="button" id="agregadiagnostico" name="agregadiagnostico" value="Agregar" onclick="location.href = 'agregarDiagnostico.php?nroAfi=<?php echo $rowTitularActivo['nroafiliado']?>&nroOrd=0&estAfi=A'"/>
					<?php } ?>
				</td>
			</tr>
		<?php
		}
		$sqlTitularInactivo = "SELECT p.nroafiliado, t.apellidoynombre, t.nrodocumento, t.cuil, t.fechabaja, p.diagnosticos FROM diabetesbeneficiarios p, titularesdebaja t WHERE p.nroorden = 0 AND p.nroafiliado = t.nroafiliado ORDER BY p.nroafiliado";
		...
		while($rowTitularInactivo = mysql_fetch_array($resTitularInactivo)) { ?>
			<tr>
				...
				<td><?php if($rowTitularInactivo['diagnosticos']!=0) {?>
							<input class="nover" type="button" id="listadiagnosticos" name="listadiagnosticos" value="Editar" onclick="location.href = 'listarDiagnosticos.php?nroAfi=<?php echo $rowTitularInactivo['nroafiliado']?>&nroOrd=0&estAfi=I'"/>
					<?php } else {?>
							<input class="nover" type="button" id="agregadiagnostico" name="agregadiagnostico" value="Agregar" onclick="location.href = 'agregarDiagnostico.php?nroAfi=<?php echo $rowTitularInactivo['nroafiliado']?>&nroOrd=0&estAfi=I'"/>
					<?php } ?>
				</td>
			</tr>
		<?php
		}
		$sqlFamiliarActivo = "SELECT p.nroafiliado, p.nroorden, k.descrip, f.apellidoynombre, f.nrodocumento, f.cuil, p.diagnosticos FROM diabetesbeneficiarios p, familiares f, parentesco k WHERE p.nroorden != 0 AND p.nroafiliado = f.nroafiliado AND p.nroorden = f.nroorden AND f.tipoparentesco = k.codparent ORDER BY p.nroafiliado, p.nroorden";
		...
		while($rowFamiliarActivo = mysql_fetch_array($resFamiliarActivo)) { ?>
			<tr>
				...
				<td>Activo</td>
				<td><?php if($rowFamiliarActivo['diagnosticos']!=0) {?>
							<input class="nover" type="button" id="listadiagnosticos" name="listadiagnosticos" value="Editar" onclick="location.href = 'listarDiagnosticos.php?nroAfi=<?php echo $rowFamiliarActivo['nroafiliado']?>&nroOrd=<?php echo $rowFamiliarActivo['nroorden']?>&estAfi=A'"/>
					<?php } else {?>
							<input class="nover" type="button" id="agregadiagnostico" name="agregadiagnostico" value="Agregar" onclick="location.href = 'agregarDiagnostico.php?nroAfi=<?php echo $rowFamiliarActivo['nroafiliado']?>&nroOrd=<?php echo $rowFamiliarActivo['nroorden']?>&estAfi=A'"/>
					<?php } ?>
				</td>
			</tr>
		<?php
		}
		$sqlFamiliarInactivo = "SELECT p.nroafiliado, p.nroorden, k.descrip, f.apellidoynombre, f.nrodocumento, f.cuil, f.fechabaja, p.diagnosticos FROM diabetesbeneficiarios p, familiaresdebaja f, parentesco k WHERE p.nroorden != 0 AND p.nroafiliado = f.nroafiliado AND p.nroorden = f.nroorden AND f.tipoparentesco = k.codparent ORDER BY p.nroafiliado, p.nroorden";
		...
		while($rowFamiliarInactivo = mysql_fetch_array($resFamiliarInactivo)) { ?>
			<tr>
				...
				<td><?php if($rowFamiliarInactivo['diagnosticos']!=0) {?>
							<input class="nover" type="button" id="listadiagnosticos" name="listadiagnosticos" value="Editar" onclick="location.href = 'listarDiagnosticos.php?nroAfi=<?php echo $rowFamiliarInactivo['nroafiliado']?>&nroOrd=<?php echo $rowFamiliarInactivo['nroorden']?>&estAfi=I'"/>
					<?php } else {?>
							<input class="nover" type="button" id="agregadiagnostico" name="agregadiagnostico" value="Agregar" onclick="location.href = 'agregarDiagnostico.php?nroAfi=<?php echo $rowFamiliarInactivo['nroafiliado']?>&nroOrd=<?php echo $rowFamiliarInactivo['nroorden']?>&estAfi=I'"/>
					<?php } ?>
				</td>
			</tr>
		<?php
	...
	<form id="moduloDiabetes" name="moduloDiabetes" method="post"  onsubmit="return validar(this)" action="buscaBeneficiario.php">
		<div align="center">
			<h2>Nuevo Ingreso</h2>
		</div>
		<div align="center">
	...
	<?php 
		...
		if ($err == 4) {
			print("<div align='center' style='color:#FF0000'><b> EL BENEFICIARIO QUE INTENTA AGREGAR YA EXISTE EN LA LISTA DE DIABETICOS </b></div>");
		}
	?>
	</div>
