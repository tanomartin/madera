...
if(strcmp($ordenbusqueda, "nroafiliado")==0) {
	$errorbusqueda = 1;
	$pagina = "moduloDiabetes.php?nroAfi=$nroafiliado&tipAfi=A";
}

if(strcmp($ordenbusqueda, "nrodocumento")==0) {
	$errorbusqueda = 2;
	if(strcmp($tipafiliado, 'T')==0) {
		$pagina = "moduloDiabetes.php?nroAfi=$nroafiliado&tipAfi=$tipafiliado";
	} else {
		$pagina = "moduloDiabetes.php?nroAfi=$nroafiliado&nroOrd=$nroorden&tipAfi=$tipafiliado";
	}
}
...
if ($noexistebeneficiario == 1)
	header("Location: moduloDiabetes.php?err=$errorbusqueda");
else
