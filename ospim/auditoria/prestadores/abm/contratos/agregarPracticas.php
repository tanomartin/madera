		...
		$("#capitulo").change(function(){
			...
			$.ajax({
				...
				$.ajax({
					...
					dataType: 'html',
					url: "getPracticas.php",
					data: {valor:valor[1], tipo:tipo, nomenclador:nomenclador, personeria:personeria, padre:valor[0]},
				}).done(function(respuesta){
					if (respuesta != 0) {
						$("#practicas").html(respuesta);
						$("#tituloPraticas").show();
		...
		$("#subcapitulo").change(function(){
			...
			if (valor == 0) { 
				...
				$.ajax({
					...
					dataType: 'html',
					url: "getPracticas.php",
					data: {valor:valor[1], tipo:tipo, personeria:personeria, padre:valor[0]},
				}).done(function(respuesta){
					if (respuesta != 0) {
						$("#practicas").html(respuesta);
						$("#tituloPraticas").show();
...
<div align="center">
  ...
  <table border="1">
	...
	<!--******************************************************************************************************************************************************************** -->	
  	...
  	<form name="agregarContrato" id="agregarContrato" onsubmit="return validar(this)" method="post" action="agregarPracticasGuardar.php?codigo=<?php echo $codigo ?>&idcontrato=<?php echo $idcontrato ?>" >
	  <h3>Prácticas para Agregar al contrato - ID <?php echo $idcontrato ?> </h3>
	  <?php if(isset($_GET['error'])) {
	  			if ($_GET['error'] == 1) { ?>
	  				<div style='color:red'><b> NO SE PUEDE COLOCAR EN EL MISMO CONTRATO DOS PRACTICAS DE LA MISMA CATEGORIA<br> CON EL MISMO CODIGO DEL MISMO NOMENCLADOR</b></div>
	  	  <?php } 
	  	  		if ($_GET['error'] == 0) {  ?>
	        		<div style='color:blue'><b>PRACTICA/S AGREGADAS CORRECTAMENTE AL CONTRATO</b></div>
	 	  <?php } 
	  	  	} ?>
	  <p>
