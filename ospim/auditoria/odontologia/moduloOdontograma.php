...
if(isset($_POST['valor']) && isset($_POST['seleccion'])) {
	...
	//BUSCO EN TITULARES
	$sqltituacti = "SELECT t.nroafiliado, t.apellidoynombre, td.descrip as tipodocumento, t.nrodocumento, t.cuil 
					FROM titulares t, tipodocumento td 
					WHERE t.$ordenbusqueda = '$valorbusqueda' and t.tipodocumento = td.codtipdoc";
	...
	//BUSCO EN FAMILIARES
	$sqlfamiacti = "SELECT f.nroafiliado, f.nroorden, f.apellidoynombre, td.descrip as tipodocumento, f.nrodocumento, f.cuil, p.descrip as parentesco 
					FROM familiares f, tipodocumento td, parentesco p 
					WHERE f.$ordenbusqueda = '$valorbusqueda' and f.tipodocumento = td.codtipdoc and f.tipoparentesco = p.codparent";
	...
	$sqltitubaja = "SELECT t.nroafiliado, t.apellidoynombre, td.descrip as tipodocumento, t.nrodocumento, t.cuil 
					FROM titularesdebaja t, tipodocumento td 
					WHERE t.$ordenbusqueda = '$valorbusqueda' and t.tipodocumento = td.codtipdoc";
	...
	$sqlfamibaja = "SELECT f.nroafiliado, f.nroorden, f.apellidoynombre, td.descrip as tipodocumento, f.nrodocumento, f.cuil, p.descrip as parentesco 
					FROM familiaresdebaja f, tipodocumento td, parentesco p 
					WHERE f.$ordenbusqueda = '$valorbusqueda' and f.tipodocumento = td.codtipdoc  and f.tipoparentesco = p.codparent";
...
function redireccion(nroafil, nroorden, tipo) {
	var direccion = "odontograma.php?tipo="+tipo+"&nroafil="+nroafil+"&nroorden="+nroorden;
...
<body bgcolor="#CCCCCC">
<div align="center">
	<form id="odontograma" name="odontograma" method="post" onsubmit="return validar(this)" action="moduloOdontograma.php">
		<p><input class="nover" type="button" name="volver" value="Volver" onclick="location.href = 'menuOdontologico.php'" /> </p>

		<h3>Odontograma por Afiliado </h3>

		<table width="150" border="0">
		  <tr>
			<td width="30"><input name="seleccion" type="radio" value="nroafiliado" checked="checked"/></td>
			<td width="120"><div align="left">Nro Afiliado</div></td>
		  </tr>
		  <tr>
...
<?php  if ($busqueda == 1) { ?>
			<p><b>Resultados Busqueda por "<?php echo $cartel ?>" </b></p>		
<?php		if($resultado == 0) { 
		  		print("<p><font color='#0000FF'><b> No Existen Afiliado con la busqueda realizada </b></font></p>");
		  	} else {  
		...
		<?php 	foreach($arrayTitulares as $titular) {?>
					<tr>
						...
						<td><input type="button" value="Consulta | Modificar" onclick="redireccion('<?php echo $titular['nroafiliado']?>','0','A')"></input></td>
					</tr>
		  <?php } ?>
		  		</tbody> 
				</table>
	<?php	}
		...
		<?php 	foreach($arrayFamiliares as $familiar) {?>
					<tr>
						...
						<td><input type="button" value="Consulta | Modificar" onclick="redireccion('<?php echo $familiar['nroafiliado']?>','<?php echo $familiar['nroorden']?>','A')"></input></td>
					</tr>
		  <?php } ?>
		  		</tbody> 
				</table>
	<?php	} 
		...
		<?php 	foreach($arrayTituBaja as $titularBaja) {?>
					<tr>
						...
						<td><input type="button" value="Consultar" onclick="redireccion('<?php echo $titularBaja['nroafiliado']?>','0','B')"></input></td>
					</tr>
		  <?php } ?>
		  		</tbody> 
				</table>
	<?php	} 
		...
		<?php 	foreach($arrayFamiBaja as $familiarBaja) {?>
					<tr>
						...
						<td><input type="button" value="Consultar" onclick="redireccion('<?php echo $familiarBaja['nroafiliado']?>','<?php echo $familiarBaja['nroorden']?>','B')"></input></td>
					</tr>
		  <?php } ?>
		  		</tbody> 
				</table>
